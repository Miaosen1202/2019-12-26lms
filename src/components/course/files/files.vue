                                ...
                                <template slot-scope="scope">
                                    ...
                                    <div v-if="scope.row.isInput">
                                        <el-input
                                            ...
                                            style="width:140px"
                                        />
                                        <!-- <a @click="scope.row.isInput = false"
                                        class="reNameTag">
        ...
        <el-dialog :visible.sync="isAddFolder" width="30%" center>
            ...
            <div style="text-align: center">
                <el-form
                    ...
                    label-width="120px"
                >
                    <el-form-item label="Folder Name" prop="addFolderName">
                        <el-input
        ...
        <el-dialog :visible.sync="isPermisson" width="40%" center>
            ...
            <div>
                ...
                <div v-show="permissonRadio == '3'" style="padding-left: 20px">
                    ...
                    line-height: 30px;
                     margin-bottom: 10px">
                     {{$t('message.Restricted_Access')}}</h1> -->
                    <el-radio-group v-model="accessRadio" class="access-box">
                        <!-- <el-radio :label="1"
                        class="radioItem">
                        {{$t('message.unlimited')}} </el-radio> -->
                        <el-radio :label="2" class="radioItem">
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            if (val) {
                ...
                this.canUpload = val.allowUpload === 1;
        ...
        cancelReName(row) {
            ...
            const tempId = row.id;
            const Bindex = this.tableData.findIndex(el => el.id === tempId);
        ...
        toReName(row) {
            ...
            const tempId = row.id;
            const aindex = this.tableData.findIndex(el => el.id === tempId);
        ...
        async reName(row) {
            const tempId = row.id;
            const tempFileName = this.tempName;
            const Cindex = this.tableData.findIndex(el => el.id === tempId);
            let tempUrlObj = {};
            if (row.fileUrl) {
        ...
        async loadNode2(node, resolve) {
            ...
            if (node.level === 0) {
                const tempPopFolderData = await this.getPopFolderData();
        ...
        moveNodeClick(data) {
            ...
            if (clickTreeId.startsWith(selectTreeId) || clickId === selectId) {
                this.canMove = false;
                this.canCopy = false;
            } else {
                this.canMove = true;
            ...
            if (moveBeforeEight !== curBeforeEight) {
                this.canMove = false;
                this.canCopy = true;
            }
        },
        ...
        async getPopFolderData() {
            ...
            const res = await this.$getData("/userFile/list", "get");
            if (res.data.code === 200) {
                let tempData = res.data.entity;
                if (this.hasCourseId) {
                    tempData = tempData.filter(
                        el =>
                            el.courseId === this.courseId && el.spaceType === 1
                    );
                }
                tempData.forEach(el => {
                    el.leaf = !el.isDirectory;
